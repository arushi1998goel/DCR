...
public class ReportTestManager {
	...
	public static synchronized ExtentTest startTest(String testName, String className) {
		...
		if (System.getProperty("AuthorName") == null || System.getProperty("AuthorName").equals("")) {
			try {
				authName = Configuration.readApplicationFile("AuthorName");
			} catch (Exception e) {				
				e.printStackTrace();
			}
		} else {
			authName = System.getProperty("AuthorName");
		}
